  ...
  return (
    <>
      <section className="relative overflow-hidden bg-slate-950 text-white">
        ...
        <div className="absolute -left-32 top-20 size-80 rounded-full bg-sky-500/10 blur-3xl" />
        <div className="absolute -right-32 bottom-0 size-96 rounded-full bg-emerald-500/10 blur-3xl" />
        <Container className="relative grid gap-12 py-16 sm:py-20 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:py-24">
          <div>
            ...
            <div className="mt-9 flex flex-col gap-3 sm:flex-row">
              ...
              <ButtonLink href="/subjects" variant="secondary" size="lg" className="border-white/20 bg-white/5 text-white hover:border-white/30 hover:bg-white/10">Explore the syllabus</ButtonLink>
            </div>
          ...
          <div className="relative mx-auto w-full max-w-xl lg:ml-auto">
            ...
            <div className="relative aspect-square sm:aspect-[1.08/1]">
              <div className="absolute left-1/2 top-1/2 grid size-32 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border border-emerald-300/30 bg-emerald-400/10 text-center shadow-[0_0_80px_rgba(52,211,153,0.18)] backdrop-blur-sm">
              ...
              {subjects.map((subject, index) => {
                ...
                return <Link key={subject.slug} href={`/subjects/${subject.slug}`} className={`group absolute ${positions[index] ?? "right-0 bottom-0"} w-40 rounded-2xl border p-4 backdrop-blur-sm transition duration-300 hover:-translate-y-1 ${hues[subject.slug] ?? "border-white/20 bg-white/10 text-white"}`}><SubjectIcon slug={subject.slug} className="size-7" /><strong className="mt-3 block text-white">{subject.name}</strong><span className="mt-1 block text-xs opacity-70">{subject.chapterCount} chapters · {subject.questionCount} Qs</span></Link>;
              })}
            </div>
          </div>
        </Container>
      ...
      <section className="py-16 sm:py-24">
        <Container>
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-3">{subjects.map((subject) => <SubjectCard key={subject.slug} subject={subject} />)}</div>
        </Container>
      </section>
      ...
      <section className="py-16 sm:py-20"><Container><div className="relative overflow-hidden rounded-[2rem] bg-slate-950 px-6 py-12 text-white sm:px-12 sm:py-14"><div className="dot-field absolute inset-0 opacity-20" /><div className="relative flex flex-col justify-between gap-8 lg:flex-row lg:items-center"><div><p className="text-sm font-bold text-emerald-400">Your next question is ready.</p><h2 className="mt-2 max-w-2xl text-3xl font-bold tracking-[-0.04em] sm:text-4xl">Don’t wait to feel prepared. Build the feeling.</h2></div><ButtonLink href="/practice" size="lg" className="shrink-0 bg-white text-slate-950 shadow-none hover:bg-emerald-50">Start practising <ArrowRightIcon className="size-4" /></ButtonLink></div></div></Container></section>
    </>
  );
}
